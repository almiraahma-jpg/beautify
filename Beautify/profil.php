...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .hero-card { background: linear-gradient(135deg, var(--pink-light) 0%, var(--bg) 100%); border-radius: 16px; padding: 28px 20px; text-align: center; box-shadow: 0 2px 12px rgba(242,151,160,0.15); margin-bottom: 16px; position: relative; overflow: hidden; }
        ...
        .avatar-circle { width: 90px; height: 90px; border-radius: 50%; background: linear-gradient(135deg, var(--pink), #e8848e); display: flex; align-items: center; justify-content: center; font-size: 38px; color: white; border: 4px solid white; box-shadow: 0 4px 16px rgba(242,151,160,0.35); margin: 0 auto; overflow: hidden; }
        .avatar-circle img { width: 100%; height: 100%; object-fit: cover; }
        .avatar-edit-btn { position: absolute; bottom: 2px; right: 2px; width: 28px; height: 28px; border-radius: 50%; background: var(--pink); color: white; border: 2px solid white; display: flex; align-items: center; justify-content: center; font-size: 12px; cursor: pointer; box-shadow: 0 2px 6px rgba(0,0,0,0.15); }
        ...
        .stat-box { flex: 1; background: rgba(255,255,255,0.7); border-radius: 10px; padding: 10px 6px; text-align: center; }
        ...
        .side-menu a { display: flex; align-items: center; gap: 10px; padding: 13px 18px; font-size: 13px; font-weight: 600; color: var(--text-muted); text-decoration: none; border-left: 3px solid transparent; transition: all 0.2s; }
        .side-menu a:hover, .side-menu a.active { color: var(--pink); background: #fff5f5; border-left-color: var(--pink); }
        ...
        .fg input, .fg select, .fg textarea { padding: 10px 14px; border: 1.5px solid var(--border); border-radius: 8px; font-size: 13px; font-family: inherit; color: var(--text); background: #fafaf8; outline: none; transition: border 0.2s; }
        .fg input:focus, .fg select:focus, .fg textarea:focus { border-color: var(--pink); background: white; }
        .fg textarea { resize: vertical; min-height: 80px; }
        .btn-save { background: var(--pink); color: white; border: none; padding: 11px 28px; border-radius: 8px; font-size: 13px; font-weight: 700; cursor: pointer; display: inline-flex; align-items: center; gap: 8px; transition: background 0.2s; font-family: inherit; }
        .btn-save:hover { background: #e07880; }
        .toast { display: none; position: fixed; top: 20px; right: 20px; z-index: 999; background: #e8f5e9; color: #2e7d32; border: 1px solid #c8e6c9; padding: 12px 20px; border-radius: 10px; font-size: 13px; font-weight: 700; box-shadow: 0 4px 16px rgba(0,0,0,0.1); }
        ...
        .riwayat-item img { width: 48px; height: 48px; border-radius: 8px; object-fit: cover; }
        ...
        @media (max-width: 768px) { .container { grid-template-columns: 1fr; } .form-row { grid-template-columns: 1fr; } }
    </style>
</head>
<body>
...
<div class="container">
    <div>
        <div class="hero-card">
            ...
            <div class="stats-row">
                <div class="stat-box"><div class="stat-val" id="statPesanan">0</div><div class="stat-label">Pesanan</div></div>
                <div class="stat-box"><div class="stat-val" id="statProduk">0</div><div class="stat-label">Produk</div></div>
                <div class="stat-box"><div class="stat-val" style="font-size:12px;" id="statSpend">Rp 0</div><div class="stat-label">Belanja</div></div>
            </div>
        </div>
        <div class="side-menu">
    ...
    <div>
        <div class="tab-panel active" id="tab-profil">
            <div class="main-card">
                <div class="main-card-header"><span style="font-size:20px;">✏️</span><h2>Edit Biodata Diri</h2></div>
                <div class="main-card-body">
                    <div class="form-row">
                        <div class="fg"><label>Nama Lengkap</label><input type="text" id="inputNama" placeholder="Nama lengkap kamu"></div>
                        <div class="fg"><label>Email</label><input type="email" id="inputEmail" placeholder="email@contoh.com"></div>
                    </div>
                    <div class="form-row">
                        <div class="fg"><label>No. Telepon</label><input type="text" id="inputTelepon" placeholder="08xxxxxxxxxx"></div>
                    ...
                    <div class="form-row">
                        <div class="fg"><label>Tanggal Lahir</label><input type="date" id="inputTglLahir"></div>
                        <div class="fg"><label>Kota</label><input type="text" id="inputKota" placeholder="Surabaya"></div>
                    </div>
                    <div class="form-row one">
                        <div class="fg"><label>Alamat Lengkap</label><textarea id="inputAlamat" placeholder="Jalan, kelurahan, kecamatan..."></textarea></div>
                    </div>
...
<script>
// ─── DATA DARI PHP SESSION ───
const serverNama  = "<?= htmlspecialchars($_SESSION['user_nama']  ?? '') ?>";
const serverEmail = "<?= htmlspecialchars($_SESSION['user_email'] ?? '') ?>";

// ─── LOAD PROFIL ───
let profil = JSON.parse(sessionStorage.getItem('beautify_profil') || '{}');

// Selalu update nama & email dari server (biar selalu sinkron sama login)
profil.nama  = serverNama  || profil.nama  || '';
profil.email = serverEmail || profil.email || '';
...
function loadRiwayat() {
    ...
    if (pesanan.length === 0) {
        container.innerHTML = `<div class="main-card"><div class="main-card-header"><span style="font-size:20px">📦</span><h2>Riwayat Pesanan</h2></div><div class="main-card-body"><div class="empty-box"><span class="icon">📦</span><p>Belum ada pesanan</p><small>Yuk mulai belanja produk beauty favoritmu!</small><br><a href="index.php" class="btn-belanja">Mulai Belanja</a></div></div></div>`;
        return;
    }
    ...
    container.innerHTML = `<div class="main-card" style="margin-bottom:0;"><div class="main-card-header"><span style="font-size:20px">📦</span><h2>Riwayat Pesanan</h2></div></div><div style="margin-top:12px;">${pesanan.map(p => `<div class="riwayat-card"><div class="riwayat-header"><div><div class="riwayat-id">🧾 ${p.kode}</div><div class="riwayat-tgl">📅 ${formatTgl(p.tanggal)}</div></div><span class="badge-status">✅ Diproses</span></div><div class="riwayat-items">${p.items.map(item => `<div class="riwayat-item"><img src="${item.img}" alt="${item.name}"><div><div class="ri-name">${item.name}</div><div class="ri-sub">${item.brand} · ${item.qty} pcs</div></div><div class="ri-price">${formatRp(item.price * item.qty)}</div></div>`).join('')}</div><div class="riwayat-footer"><div class="riwayat-meta">via ${payLabel[p.pembayaran] || p.pembayaran} · Ongkir: ${p.ongkir === 0 ? 'GRATIS' : formatRp(p.ongkir)}</div><div class="riwayat-total">${formatRp(p.total)}</div></div></div>`).join('')}</div>`;
}
